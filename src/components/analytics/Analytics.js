...
const Analytics = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`https://quizze-1.onrender.com/api/quiz`);
  ...
  const handleDeleteConfirm = async () => {
    try {
      ...
      const response = await fetch(
        `https://quizze-1.onrender.com/api/quiz/${quizToDelete}`,
        {
          method: "DELETE",
          headers: {
  ...
  const handleShare = (quizId) => {
    const linkToCopy = `https://quizze-1.onrender.com/api/quiz/${quizId}`;
      ...
      {deleteData && (
        <div className={style.main}>
          <h1 style={{ textAlign: "center" }}>
            Are your confirm you want to delete??
          </h1>
          <div
            style={{
              width: "80%",
          ...
          >
            <button className={style.btn} onClick={handleDeleteConfirm}>
              confirm Delete
            </button>
            <button onClick={() => setDeleteData(false)} className={style.btn}>
              cancel
            </button>
          </div>
        </div>
